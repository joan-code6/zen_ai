...
class AppState extends ChangeNotifier {
  ...
  Future<bool> updateDisplayName(String newDisplayName) async {
    ...
    if (currentSession == null) {
      _setError('Please sign in to update your display name.');
      return false;
    }
    ...
    try {
      ...
      _applySession(updatedSession);
      await UserPreferences.setAuthSession(updatedSession);
      _setInfo('Display name updated to $resolvedName.');
      return true;
    } on BackendException catch (e) {
      _setError(e.message);
    } catch (e) {
  ...
  Future<Chat?> getChat(String chatId) async {
    ...
    notifyListeners();
    try {
      final chat = await BackendService.getChat(
        chatId: chatId,
        uid: currentUid,
      );
  ...
  Future<Chat?> updateChat({
    ...
    try {
      ...
      final existingMessages =
          _chatCache[chatId]?.messages ?? const <ChatMessage>[];
      _upsertChat(
        chat.copyWith(messages: List.of(existingMessages)),
        moveToTop: false,
      );
      notifyListeners();
      return chat;
  ...
  void _upsertChat(Chat chat, {required bool moveToTop}) {
    ...
    if (existing != null) {
      final mergedMessages = chat.messages.isNotEmpty
          ? chat.messages
          : existing.messages;
      final mergedFiles = chat.files.isNotEmpty ? chat.files : existing.files;
